...
#define INSOP(n) insn->detail->tms320c67x.operands[n]
#define INSCC    insn->detail->tms320c67x.cc
...
int tms320_c67x_disassemble(RzAsm *a, RzAsmOp *op, const ut8 *buf, int len, void *c67x) {
	TMSContext *ctx = (TMSContext *)c67x;
	...
	if (!ctx->handle) {
		...
		ret = cs_open(CS_ARCH_TMS320C67X, mode, &ctx->handle);
		if (ret) {
			goto fin;
		}
		ctx->omode = 0;
...
static void tms320_c67x_opex(RzStrBuf *buf, csh handle, cs_insn *insn) {
	int i;
	PJ *pj = pj_new();
	if (!pj) {
		return;
	...
	pj_o(pj);
	pj_ka(pj, "operands");
	cs_tms320c67x *x = &insn->detail->tms320c67x;
	for (i = 0; i < x->op_count; i++) {
		cs_tms320c67x_op *op = x->operands + i;
		pj_o(pj);
		switch (op->type) {
		case TMS320C67X_OP_REG:
			pj_ks(pj, "type", "reg");
			pj_ks(pj, "value", cs_reg_name(handle, op->reg));
			break;
		case TMS320C67X_OP_IMM:
			pj_ks(pj, "type", "imm");
			pj_ki(pj, "value", op->imm);
			break;
		case TMS320C67X_OP_MEM:
			pj_ks(pj, "type", "mem");
...
int tms320_c67x_op(RzAnalysis *a, RzAnalysisOp *op, ut64 addr, const ut8 *buf, int len, RzAnalysisOpMask mask, void *c67x) {
	...
	if (n < 1) {
		...
		if (mask & RZ_ANALYSIS_OP_MASK_OPEX) {
			tms320_c67x_opex(&op->opex, ctx->handle, insn);
		}
		op->size = insn->size;
		op->id = insn->id;
		switch (insn->id) {
		case TMS320C67X_INS_INVALID:
			op->type = RZ_ANALYSIS_OP_TYPE_ILL;
			break;
